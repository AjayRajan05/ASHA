...
def _fail_patch_processing(exc: Exception, verbose: bool = False) -> None:
    if verbose:
        print(f"[PrivySHA] SDK patch processing failed: {exc}")
...
def auto_patch(
    ...
    Args:
        ...
        mode: PrivySHA processing mode (default ``strict`` for global SDK patching)
    ...
    if "openai" in providers:
        try:
            if not _check_version_compatibility("openai", verbose):
                ...
                result = _patch_openai(privysha_process, mode, verbose)
                if result["success"]:
                    patches_applied += 1
    ...
    if "anthropic" in providers:
        try:
            if not _check_version_compatibility("anthropic", verbose):
                ...
                result = _patch_anthropic(privysha_process, mode, verbose)
                if result["success"]:
                    patches_applied += 1
    ...
    if "google.generativeai" in providers:
        try:
            result = _patch_google_generativeai(privysha_process, mode, verbose)
            if result["success"]:
                patches_applied += 1
    ...
    if "huggingface" in providers:
        try:
            result = _patch_huggingface(privysha_process, mode, verbose)
            if result["success"]:
                patches_applied += 1
...
def _patch_openai(
    privysha_process: Callable, mode: str, verbose: bool = False
) -> Dict[str, Any]:
    ...
    if _parse_major_version(version) >= 1:
        return _patch_openai_v1(privysha_process, mode, verbose)

    return _patch_openai_legacy(privysha_process, mode, verbose)
    ...
    def patched_messages_create(self: Any, *args: Any, **kwargs: Any) -> Any:
        ...
        try:
            ...
            kwargs["messages"] = _process_anthropic_messages(
                messages, privysha_process, mode, verbose
            )
            return original_create(self, *args, **kwargs)
        except Exception as e:
            _fail_patch_processing(e, verbose)
...
def _process_anthropic_messages(
    messages: List[Any], privysha_process: Callable, mode: str, verbose: bool
) -> List[Any]:
    ...
    for idx, message in enumerate(messages):
        if idx == last_user_idx and isinstance(message, dict) and "content" in message:
            ...
            result = privysha_process(str(content), mode=mode, debug=verbose)
