...
auto_patch_module = importlib.import_module("privysha.utils.auto_patch")

from privysha.utils.auto_patch import (
...
def test_openai_v1_patch_with_mock_module(monkeypatch):
    ...
    monkeypatch.setattr(
        auto_patch_module,
        "_patch_openai",
        lambda proc, verbose=False: auto_patch_module._patch_openai_v1(
            fake_process, verbose
        ),
    )
...
def test_auto_patch_fail_safe_on_process_error(monkeypatch):
    class Completions:
        @staticmethod
        def create(self, *args, **kwargs):
    ...
    monkeypatch.setattr(
        auto_patch_module,
        "_patch_openai",
        lambda proc, verbose=False: auto_patch_module._patch_openai_v1(boom, verbose),
    )

    auto_patch(providers=["openai"])
    messages = Completions.create(
        None,
        messages=[{"role": "user", "content": "original@example.com"}],
    )
    assert messages[0]["content"] == "original@example.com"
    auto_patch(enable=False)
...
def test_anthropic_patch_processes_last_user_message(monkeypatch):
    ...
    auto_patch_module._patch_anthropic(fake_process, verbose=False)
...
def test_gemini_patch_processes_generate_content(monkeypatch):
    ...
    auto_patch_module._patch_google_generativeai(fake_process, verbose=False)
...
def test_huggingface_patch_processes_pipeline_call(monkeypatch):
    ...
    auto_patch_module._patch_huggingface(fake_process, verbose=False)
