...
# utils/__init__.py imports the `auto_patch` *function* under the same name as
# the submodule, shadowing the module when accessed via attribute lookup.
# Use importlib to get the actual module object.
ap_mod = importlib.import_module("privysha.utils.auto_patch")
...
def test_auto_patch_verbose_does_not_raise(monkeypatch):
    ...
    monkeypatch.setattr(ap_mod, "_patch_openai", lambda proc, verbose=False: None)
    monkeypatch.setattr(ap_mod, "_patch_anthropic", lambda proc, verbose=False: None)
    monkeypatch.setattr(
        ap_mod,
    ...
    monkeypatch.setattr(
        ap_mod, "_patch_huggingface", lambda proc, verbose=False: None
    )
